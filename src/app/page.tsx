...
export default function Home() {
  ...
  const handleDetection = useCallback(async () => {
    try {
      await processVideo(processFrames);
      setDetectionComplete(true);
    } catch (error) {
      console.error('Error during detection:', error);
    }
  }, [processVideo, processFrames]);
  ...
  }, [metadata, detections, getSelectedTrack, processReframing]);
  ...
  const handleExport = useCallback(async (options: ExportOptions) => {
    ...
    try {
      const blob = await exportVideo(videoElement, metadata, options);
      
      // Download the file
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `reframed_${videoFile?.name || 'video'}.${options.format}`;
      a.click();
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">AI-powered video reframing</p>
              </div>
            </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400">
                Transform your videos for any social media platform with AI
              </p>
            </div>
